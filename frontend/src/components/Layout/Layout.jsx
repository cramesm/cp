        ...
        <div className="flex min-h-screen bg-[#f2f2f2]">
            ...
            <aside className="hidden md:block w-[250px] h-screen bg-[#213448] fixed top-0 left-0 pt-5 z-[1000]">
                ...
                <ul className="list-none p-0">
                    {['', 'requests', 'transactions', 'notifications', 'profile'].map((path) => {
                        const labels = {
                            '': 'Dashboard',
                        ...
                        return (
                            <li key={path} className="px-5 py-2.5">
                                <NavLink 
                                    to={`/${path}`} 
                                    className={({ isActive }) => 
                                        `block text-white no-underline font-semibold py-3.5 px-5 rounded-[10px] transition-colors ${isActive ? 'bg-white text-black' : 'hover:bg-white hover:text-black'}`
                                    }
                                >
                                    {labels[path]}
                                </NavLink>
