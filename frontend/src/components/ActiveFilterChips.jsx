...
const ActiveFilterChips = ({ filters, onRemove }) => {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 mt-4 mb-2">
      <span className="text-xs font-bold text-gray-600 uppercase tracking-wider mr-1">Active Filters:</span>
      {activeFilters.map((filter, index) => (
        ...
        >
          <span><span className="text-blue-700">{filter.label}:</span> {filter.value}</span>
          <button 
            onClick={() => onRemove(filter.key)}
            className="p-0.5 hover:bg-blue-200 rounded-full transition-colors text-blue-700 hover:text-blue-900"
